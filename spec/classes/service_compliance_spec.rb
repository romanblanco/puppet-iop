  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with default parameters' do
        ...
        it { should contain_podman__quadlet('iop-service-compliance-backend-sidekiq') }
      end
      ...
      context 'with ensure => absent' do
        ...
        it { should contain_podman__quadlet('iop-service-compliance-backend-sidekiq').with_ensure('absent') }
      end
      ...
      context 'secret subscription behavior' do
        let(:compliance_services) do
          [
            ...
            'iop-service-compliance-backend-sidekiq',
          ]
        end

        let(:database_secrets) do
          [
            'Podman::Secret[iop-service-compliance-backend-database-username]',
            'Podman::Secret[iop-service-compliance-backend-database-password]',
            'Podman::Secret[iop-service-compliance-backend-database-name]',
            'Podman::Secret[iop-service-compliance-backend-database-host]',
            'Podman::Secret[iop-service-compliance-backend-database-port]',
          ]
        end

        it 'should create all required database secrets' do
          should contain_podman__secret('iop-service-compliance-backend-database-username')
          should contain_podman__secret('iop-service-compliance-backend-database-password')
          should contain_podman__secret('iop-service-compliance-backend-database-name')
          should contain_podman__secret('iop-service-compliance-backend-database-host')
          should contain_podman__secret('iop-service-compliance-backend-database-port')
        end
        ...
        it 'should ensure all compliance services subscribe to database secrets' do
          compliance_services.each do |service|
            should contain_podman__quadlet(service)
              .that_subscribes_to('Podman::Secret[iop-service-compliance-backend-database-password]')
          end
        end
      end
    end
